...
def main():
    ...
    parser.add_argument('--DisableStyles', type=bool, default=False, help='当此项为 True 时，将禁用圆点、方点样式的生成。')
    ...
    # shutil.copy(path_define.src_dir.joinpath('ZLabsGeoPix_16px_SC_patch.kbitx'), path_define.data_dir)

    # 合并字形，生成对应标准字形的完整版本
    for region in ['SC']:
        # advanced_merge_kbitx_files(path_define.src_dir.joinpath(f'ZLabsGeoPix_16px_SC_patch.kbitx'),
        #                            path_define.src_dir.joinpath(f'ZLabsGeoPix_16px_{region}_diff.kbitx'),
        #                            path_define.src_dir.joinpath(f'flags_{region}.txt'),
        #                            path_define.data_dir.joinpath(f'ZLabsGeoPix_16px_{region}.kbitx'))
        merge_kbitx_files(path_define.src_dir.joinpath(f'16.kbitx'),
                          path_define.src_dir.joinpath(f'ZLabsGeoPix_16px_SC_patch.kbitx'),
                          path_define.data_dir.joinpath(f'ZLabsGeoPix_16px_SC.kbitx'))


    # 生成字体
    for language_flavor in options.language_flavors:
        kbit_font = KbitFont.load_kbitx(path_define.data_dir.joinpath(f'ZLabsGeoPix_16px_{language_flavor}.kbitx'))
        ...
        for style in outlineStyles:
            ...
            print(f'Creating ZLabsGeoPix_16px_{outputCode}_{language_flavor.upper()}.ttf, please wait…')
            ttf_font.save(path_define.outputs_dir.joinpath(f'ZLabsGeoPix_16px_{outputCode}_{language_flavor.upper()}.ttf'))
            print(f'Successfully created ZLabsGeoPix_16px_{outputCode}_{language_flavor.upper()}.ttf')

            ttf_font.flavor = 'woff2'
            print(f'Creating ZLabsGeoPix_16px_{outputCode}_{language_flavor.upper()}.ttf.woff2, please wait…')
            ttf_font.save(path_define.outputs_dir.joinpath(f'ZLabsGeoPix_16px_{outputCode}_{language_flavor.upper()}.ttf.woff2'))
            print(f'Successfully created ZLabsGeoPix_16px_{outputCode}_{language_flavor.upper()}.ttf.woff2')
